...
const checkInventory = async (items) => {
  ...
  return axios.get(`${inventoryServiceUrl}/inventory/check`, { 
    params: { 
...
const releaseInventory = async (items) => {
  ...
  return axios.post(`${inventoryServiceUrl}/inventory/release`, { 
    items,
...
const processPayment = async (paymentInfo) => {
  ...
  return axios.post(`${paymentServiceUrl}/payments/authorize`, paymentInfo);
};
...
// Create circuit breaker for shipping service
...
const createShipment = async (shipmentInfo) => {
  // Call the actual shipping service
  const shippingServiceUrl = process.env.SHIPPING_SERVICE_URL || 'http://localhost:3003';
  
  return axios.post(`${shippingServiceUrl}/shipping`, shipmentInfo);
};

const shippingServiceBreaker = new CircuitBreaker(createShipment, shippingServiceOptions);

// Add listeners
shippingServiceBreaker.on('open', () => {
  metrics.circuitBreakerState.set({ service: 'shipping' }, 1);
  logger.warn('Shipping service circuit breaker: OPEN');
  console.log('Shipping service circuit breaker: OPEN');
});

shippingServiceBreaker.on('close', () => {
  metrics.circuitBreakerState.set({ service: 'shipping' }, 0);
  logger.info('Shipping service circuit breaker: CLOSED');
  console.log('Shipping service circuit breaker: CLOSED');
});

shippingServiceBreaker.fallback(async (shipmentInfo) => {
  logger.warn('Using fallback for shipping service', { orderId: shipmentInfo.orderId });
  console.log('Using fallback for shipping service');
  
  // Record for later retry and provide a temporary tracking number
  const tempTrackingNumber = `TEMP-${Date.now()}-${shipmentInfo.orderId}`;
  return {
    data: {
      id: `temp-${shipmentInfo.orderId}`,
      trackingNumber: tempTrackingNumber,
      status: 'PENDING_RETRY',
      usingFallback: true,
      message: 'Shipment will be processed when the service is available'
    }
  };
});
