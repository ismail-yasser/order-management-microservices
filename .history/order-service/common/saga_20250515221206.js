...
const { inventoryServiceBreaker, paymentServiceBreaker } = require('./resilience');
...
const executeOrderPlacementSaga = async (orderData, orders) => {
  ...
  try {
    ...
    try {
      ...
      const inventoryResult = await inventoryServiceBreaker.fire(orderData.items);
      logger.info(`Inventory check completed in ${Date.now() - inventoryStart}ms`);
      
      if (!inventoryResult.data.available && !inventoryResult.data.usingFallback) {
        recordEvent('INVENTORY_FAILED', { sagaId, orderId });
      ...
      try {
        ...
        recordEvent('PAYMENT_FAILED', { sagaId, orderId, error: paymentError.message });
        
        // Compensating action: release inventory
        try {
          await inventoryServiceBreaker.fire({ 
            action: 'release', 
            items: orderData.items 
          });
