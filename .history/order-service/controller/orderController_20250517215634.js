...
// Add a tracking endpoint to get shipment details
exports.trackShipment = async (req, res) => {
    try {
        ...
        try {
            ...
            // Fetch tracking information from shipping service
            const trackingResult = await axios.get(
                `${shippingServiceUrl}/shipping/track/${order.shipping.trackingNumber}`
            );
            
            logger.info(`Tracking info fetched for order ${order.id}`, {
                orderId: order.id,
                trackingNumber: order.shipping.trackingNumber
            });
            ...
            // Provide basic tracking even if shipping service is down
            logger.error(`Error getting tracking info from shipping service: ${error.message}`);
            res.json({
                order: {
                    id: order.id,
                    status: order.status,
                ...
                tracking: {
                    ...
                    status: 'TRACKING_SERVICE_UNAVAILABLE',
                    message: 'Tracking service is currently unavailable. Please try again later.'
                }
            });
        }
    } catch (error) {
        logger.error('Error tracking shipment', { error: error.message, orderId: req.params.id });
        res.status(500).json({ error: 'Internal server error' });
    }
};
